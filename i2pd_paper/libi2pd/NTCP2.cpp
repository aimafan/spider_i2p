...
namespace i2p
{
	namespace transport
	{
																								 ...
																								 m_IsSending(false), m_IsReceiving(false), m_NextPaddingSize(16)
		{
			...
			{
				// LogToFile("构造函数 host = " + addr->host.to_string() + "; port = " + std::to_string(addr->port));
		...
		NTCP2Session::~NTCP2Session()
		{
			...
			// LogToFile("析构函数 host = " + host + "; port = " + port);
		...
		void NTCP2Session::Terminate()
		{
			std::string host = m_RemoteEndpoint.address().to_string();
			std::string port = std::to_string(m_RemoteEndpoint.port());
			// LogToFile("发送terminate消息 host = " + host + "; port = " + port);
			if (!m_IsTerminated)
			{
				m_IsTerminated = true;
		...
		void NTCP2Session::Established()
		{
			std::string host = m_RemoteEndpoint.address().to_string();
			std::string port = std::to_string(m_RemoteEndpoint.port());
			// LogToFile("会话建立成功 host = " + host + "; port = " + port);
		...
		void NTCP2Session::CreateNextReceivedBuffer(size_t size)
		{
			...
			std::string port = std::to_string(m_RemoteEndpoint.port());
			// 这里的消息是解密后的还是raw的呢？
			// LogToFile("接收到的消息长度 host = " + host + "; port = " + port + "  接收到消息的长度为size = " + std::to_string(size));
		...
		void NTCP2Session::HandleSessionRequestSent(const boost::system::error_code &ecode, std::size_t bytes_transferred)
		{
			...
			// LogToFile("收到第一次握手 host = " + host + "; port = " + port + "; size = " + size);
		...
		void NTCP2Session::HandleSessionRequestPaddingReceived(const boost::system::error_code &ecode, std::size_t bytes_transferred)
		{
			...
			std::string port = std::to_string(m_RemoteEndpoint.port());
			std::string size = std::to_string(bytes_transferred);
			// LogToFile("处理接收到的会话请求填充消息 host = " + host + "; port = " + port + "; size = " + size);
			if (ecode)
			{
		...
		void NTCP2Session::SendSessionCreated()
		{
			...
			// std::string size = std::to_string(m_Establisher->m_SessionCreatedBufferLen);
		}
		...
		void NTCP2Session::HandleSessionCreatedPaddingReceived(const boost::system::error_code &ecode, std::size_t bytes_transferred)
		{
			...
			std::string port = std::to_string(m_RemoteEndpoint.port());
			std::string size = std::to_string(bytes_transferred);
			// LogToFile("处理接收到的会话创建填充消息 host = " + host + "; port = " + port + "; size = " + size);
			if (ecode)
			{
		...
		void NTCP2Session::SetSipKeys(const uint8_t *sendSipKey, const uint8_t *receiveSipKey)
		{
			std::string host = m_RemoteEndpoint.address().to_string();
			std::string port = std::to_string(m_RemoteEndpoint.port());
			// LogToFile("设置SipKeys host = " + host + "; port = " + port);
#if OPENSSL_SIPHASH
		...
		void NTCP2Session::ClientLogin()
		{
			...
			std::string size = std::to_string(m_Establisher->m_SessionRequestBufferLen);
			// LogToFile("客户端 host = " + host + "; port = " + port + "; size = " + size);
			SendSessionRequest();
		}
		...
		void NTCP2Session::ServerLogin()
		{
			...
			m_Establisher->CreateEphemeralKey();
			std::string size = std::to_string(m_Establisher->m_SessionRequestBufferLen);
			// LogToFile("服务器 host = " + host + "; port = " + port + "; size = " + size);
		...
		void NTCP2Session::SendI2NPMsgs(std::vector<std::shared_ptr<I2NPMessage>> &msgs)
		{
			...
			for (auto &it : msgs)
			{
				...
				// block header
				buf -= 3;
				buf[0] = eNTCP2BlkI2NPMessage; // blk，表示是I2NP消息
				...
				// LogToFile("发送I2NP消息 host = " + host + "; port = " + port + "; type = " + std::to_string(it->GetTypeID()) + "; size = " + std::to_string(len));
			}
		...
		void NTCP2Session::SendRouterInfo()
		{
			...
			// LogToFile("发送RouterInfo消息 host = " + host + "; port = " + port + "; size = " + std::to_string(riLen + payloadLen));
			LogToFile("发 ; " + host + " ; " + port + " ; 600 ; " + std::to_string(payloadLen));
		}
		...
		void NTCP2Session::SendTermination(NTCP2TerminationReason reason)
		{
			...
			// LogToFile("发 host = " + host + "; port = " + port + "; size = " + std::to_string(paddingSize + 12));
